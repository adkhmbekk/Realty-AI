...
class Settings(BaseSettings):
    ...
    # ИНТЕРАКТИВНЫЙ импорт по ссылке (риэлтор смотрит и правит форму) — тут важна
    # точность, объём крошечный, поэтому полная flash.
    import_ai_model: str = "gemini-2.5-flash"
    # МАССОВЫЙ/ФОНОВЫЙ импорт гонит тысячи постов, и их никто не вычитывает на
    # лету — берём более дешёвую flash-lite (в ~4-5 раз дешевле по выходу),
    # качества для пакетного разбора достаточно. Если точность просядет — вернуть
    # сюда "gemini-2.5-flash" (одна строка).
