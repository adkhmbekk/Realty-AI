...
# Сотрудник считается «в сети», если последний heartbeat был не дольше этого назад
# (фронтенд пингует ~раз в минуту; 3 минуты терпят пару пропущенных пингов).
_ONLINE_WITHIN = timedelta(minutes=3)
...
def activity(db: Session, agency_id: int) -> AgencyActivityOut:
    ...
    def _is_online(seen) -> bool:
        s = _as_utc(seen)
        return s is not None and (now - s) <= _ONLINE_WITHIN
