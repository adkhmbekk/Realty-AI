...
# ── Присутствие «в сети» (секунды) ───────────────────────────────────────────
# Фронт пингует раз в 30с. online = свежий heartbeat: 75с терпят пару пропущенных
# пингов (2.5 интервала), но гаснут быстро после реального выхода (~60-75с), а не
# «висят онлайн» минутами.
ONLINE_WITHIN = timedelta(seconds=75)
# recent («был только что»): ~первая минута ПОСЛЕ того, как человек вышел
# (heartbeat перестал приходить). Дальше показываем точное время выхода.
RECENT_WITHIN = timedelta(seconds=135)
