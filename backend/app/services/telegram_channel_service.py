...
# Сколько постов разбираем ИИ за ОДИН запрос (одну страницу). Держим небольшим:
# Gemini изредка перегружается (503) и мы ТЕРПЕЛИВО его ждём (повтор + пауза), а с
# маленькой порцией один HTTP-запрос всё равно не виснет дольше таймаута — фронтенд
# идёт страницами с паузами; импорт идёт чуть дольше, но не «отказывает».
_MAX_AI_PER_PAGE = 3
