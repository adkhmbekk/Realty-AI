...
def _client_ip(request: Request) -> str:
    ...
    ОСНОВНОЙ путь (H1): наш Caddy сам вычисляет реальный IP клиента (доверяя
    X-Forwarded-For только от внутренних адресов, trusted_proxies) и кладёт его в
    заголовок X-Real-Client, ПЕРЕЗАПИСЫВАЯ любое присланное клиентом значение —
    поэтому доверять ему безопасно. Так rate-limit/аудит идут по НАСТОЯЩЕМУ
    клиенту, а не по одному IP туннеля на всех.

    ЗАПАСНОЙ путь: если заголовка нет (прямое подключение/старый Caddy) —
    отсчитываем settings.trusted_proxy_count позиций СПРАВА в X-Forwarded-For
    (левые элементы клиент может дописать сам). Нет и его — адрес соединения.
    """
    real = request.headers.get("x-real-client")
